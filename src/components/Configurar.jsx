import React, { useState, useEffect } from 'react';
       ...
       function Configurar() {
           ...
           useEffect(() => {
               ...
               if (tokenParam) {
                   const tokenRef = doc(db, 'tokens', tokenParam);
                   getDoc(tokenRef).then(docSnap => {
                       ...
                       setError('Erro ao verificar token: ' + err.message);
                       setLoading(false);
                   });
               } else {
               ...
               auth.onAuthStateChanged(user => {
                   ...
                   if (user) {
                       ...
                       getDoc(userRef).then(docSnap => {
                           if (docSnap.exists()) {
                               const restaurantId = docSnap.data().restaurantId;
                               const restaurantRef = doc(db, 'restaurants', restaurantId);
                               getDoc(restaurantRef).then(rDoc => {
                                   if (rDoc.exists()) {
           ...
           const handleSubmit = async e => {
               ...
               try {
                   let logoUrl = '';
                   if (restaurant.logo) {
                       const storageRef = ref(storage, `restaurants/${user.email}/${restaurant.logo.name}`);
                       await uploadBytes(storageRef, restaurant.logo);
                   ...
                   if (existingDoc.exists()) {
                       ...
                       while (true) {
                           ...
                           const checkDoc = await getDoc(doc(db, 'restaurants', restaurantId));
                   ...
                   await setDoc(doc(db, 'users', user.email), { restaurantId });
